...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  const newArr = input.map((ele) =>
    ele.filter((el) => typeof el === "number" && el % 5 === 0)
  );
  return newArr.map((ele) => ele.map((el) => Math.pow(2, el)));
};
