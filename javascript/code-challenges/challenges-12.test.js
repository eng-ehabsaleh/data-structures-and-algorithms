...
const validatePin = (pin) => {
  // Solution code here...
  const dig = String(pin).match(/([0-9])/g);
  const not = String(pin).match(/([^0-9])/g);
  return dig ? (dig.length === 4 && !not ? true : false) : false;
};
...
const validateWord = (word) => {
  ...
  const charCount = word.match(/\w/g);
  return charCount
    ? charCount.length >= 5 && charCount.length < 10
      ? true
      : false
    : false;
};
...
const validateEmail = (email) => {
  // Solution code here...
  const validate = /^\w+\.?\w+@\w+\.(net|com|org)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  const valNumber = phoneNumber.match(
    /\(?([0-9]{3})\)?[-.●]?[\s]?([0-9]{3})[-.●]?[\s]?([0-9]{4})/g
  );
  if (!valNumber) return false;
  if (valNumber[0].length !== phoneNumber.length) return false;
  return valNumber[0].match(/\(/)
    ? valNumber[0].match(/\)/)
      ? true
      : false
    : true;
